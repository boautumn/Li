...
{
    if(1==i)	SDA_H;	             //准备好SDA电平状态，不应答
    else	SDA_L;					//准备好SDA电平状态，应答 	
	  SCL_H;	                //拉高时钟线
    IIC_delay();                 //延时
    SCL_L;                  //拉低时钟线
    IIC_delay();   
...
{
	I2C_Start();  //起始信号
	Send_Byte(Slave_Address);   //发送设备地址+写信号
	I2C_WaitAck();
	Send_Byte(REG_Address);     //发送存储单元地址
 	I2C_WaitAck();
	I2C_Start();//起始信号         
	Send_Byte(Slave_Address+1); //发送设备地址+读信号
 	I2C_WaitAck();
	while(length-1)
	{
...
{	
  I2C_Start(); 							//起始信号         
  Send_Byte(oldAddress); //发送设备地址+写信号
 	I2C_WaitAck();
		//	delay_1us(40); 
	Send_Byte(0xaa);    		//指令
	I2C_WaitAck();
