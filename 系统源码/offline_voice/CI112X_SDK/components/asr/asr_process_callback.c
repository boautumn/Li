...
#if USE_CWSL
char dtw_asr_result_sucess_flag = 0;
uint32_t vad_start_time = 0;
#endif
/*******************************************************************************
    *功能：vad start 函数调用
...
int vadstart_callback(unsigned int *pdata, int line)
{
    ...
    vad_light_on();
#if USE_CWSL
    dtw_asr_result_sucess_flag = 0;
    vad_start_time = xTaskGetTickCount();
#endif
    return 0;
}
...
#if USE_CWSL
	vad_end_time = xTaskGetTickCount();
#endif
    vad_light_off();
    return 0;
}
...
int asr_result_callback(callback_asr_result_type_t *asr)
{
    ...
    #if USE_CWSL
    //cwls 注册和删除模式下，不响应asr识别结果
    if(dtw_asr_result_sucess_flag == 1)
    {
        mprintf("cwls asr success \n");
        return 0;
    }
    #endif
